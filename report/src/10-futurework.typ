...
- *Uniform and constant delivery delay.* The delivery component of the incubation time, `propagation_delay` ($Delta_p$), is uniform across edges and degenerate in the reported run (@tbl-res-config), which removes the edge-level heterogeneity of the platform-side delay only: the rest of the incubation time is generated by the timeline queue rather than sampled (@sec-model-incubation). $arrow.r$ a config change rather than a rewrite: `propagation_delay` already accepts a distribution, so a per-edge or heavy-tailed delivery law can be fitted from cascade timings --- but it moves only the delivery component, as the heavy tail of the effective transmission time comes from the session and queue dynamics.
