...
The first version served as a proof of concept implementation and design, and was where most of the needed utilities but not part of the main program were developed, such as the Heap implementation, the _distributions_ library with the Exponential, Normal and Categorical implementation, as well as the runtime dynamic dispatch JSON config file ---despite not being near it's final form, but very functional---, in order to iterate quickly with the parameters.

The first version did not even aim to implement the same behaviour ---therefore not correct--- as the final simulation, as the timeline implements a chronological timeline (min-heaps) not a reverse one (max-heap), so the simulation was serving the older post the user had not seen, not the most recent one. To simplify it even further, the users were assumed to be homogeneous and behave all equally. This simplifications allowed to test the propagate behaviour, as well as the essential architecture of the DES simulation with a very similar data structures as the final version will end up having.
...
To change every user timeline into a max-heap was the most complicated part of bringing the model into life, for every user now had to disconnect from the simulation for their timeline to be refilled. That introduces plenty of bugs and misbehaviours with "time traveling" errors ---time causality violations--- such a repost made to a post before the post arrived to a user timeline; the propagate event @sec-design-sources-propagate was introduced to isolate those bugs, as the old mechanism directly inserted posts into the user timelines ---was nor a very good idea nor a good design decision.
...
Some additional mechanics were introduced at this version, such as notifications (allowed a user to do an interaction while offline) or quoting (two posts where appeared at the user timeline) but were dropped due to scope and design reasons, see them described on appendix @apx-mechanics.

Lastly, a small effort to refactor and improve performance was made, introducing different memory allocators depending on the task (see @apx-impl-memory), rewriting suboptimal code an tiding up the codebase.

==== v4: Final Relase 

Contains the same features of the v3 simulation with performance optimizations regarding memory management. Although might sound weird to the reader, the main perfromance improvements are not introduced in the last version as a refactor, as performance is a consequence of good through design from the beginning of the project.

The main feature of the last version is multithreading execution with a predefined number of workers (#todo[make a section on this appendix about this]) while ensuring reproducibility, as well as the creation of the pipeline to be able to schedule as many executions as flawlessly needed.
...
In Zig, Heap memory is managed with `Allocators` #todo[cite the allocator std], and each of them follows different memory strategies, and will be more or less suited in different situations. The main two used in the simulation are the following:
+ *General Purpose Allocator* #todo[cite DebugAllocator std]: reserves memory in a linear and contiguous fashion. Requires an interruption of the program execution for the kernel to provide the memory addresses. Can be resized if there is free memory adjacent to the allocation. It's the `malloc` and `realloc` equivalent in C.
+ *Arena Allocator* #todo[cite arena allocator std]: reserves a large pool of memory (just one `SYSCALL`), and controls which portion of the memory is used by just incrementing an offset. As the whole memory is already allocated upfront, very few kernel interruptions to ask for more memory are needed during execution. 
@zig-std-heap
...
The JSON containing all the distinct users and their followers is loaded in its entirety in an arena, `json_arena`. Then, the information is used to create an instance of `Topology` struct, which creates the CSR representation of the graph (see @apx-impl-csr), as well as having as much memory needed for all the specific user information, as explained in user entity on @sec-design-entities. Once the `Topology` struct is created with a different arena, we deinit `json_arena` to have that memory free for another uses, as the JSON topology is not needed anymore.

The main disadvantages of arenas are resizing and growing. When an item gets freed from an arena it is not freed as returning it back to the OS, but the offset of the arena grows marking the slot occupied with that unusable. Freeing a lot can lead to fragmentation issues as well as resizing overhead, at to resize an entire arena is time consuming for the OS.
...
The parsing of the distributions is a simple parser implemented using the lower JSON api #todo[refecente the std lower json] which enforces every defined rule, as limiting the `user_policy` to a categroical distribution, or all the time distributions to continuous distributions..

=== Runtime Dynamic Distribution Dispatch 

Being able to achive a generic JSON configuration file pays a performance penalty, as the program has to decide from which of the stated distributions should sample when executes `inter_action_time.sample()`. This is called Runtime ---does happen while the program is running--- dynamic dispatch ---serving from multiple options dynamically--- and it's a form of polymorphism. This is achieved internally by the distributions library the simulation depends upon with the generic type `DiscreteDistribution` and `ContinuousDisctribution` as seen in @code-simconf.
...
In less techincal computer science words: everytime `sample()` is called over the configuration file, a switch to know which distribution should be sampled is executed, and then the `sample()` method from the proper distribution is called.
...
The problem of loading a file into a structure is, therefore, a deserialization problem. When reading a human readible format ---all of those that store information with strings--- two deserialization must be performed: from string to binary, and from binary into the structure. Then, whenever is needed to maximize for performance, the non readible formats should be prioritized, such as parquet and raw binary.
...
Despite the author willingess to create software from scratch (#todo[cite easy args and distributions]), to code a competent parquet reader is out of scope for this project. Desptie some workarounds to get Zig a Parquet reader (import the DuckDB driver as a C++ cross compiled library) they involve a lot of extra code and a dependency, so we write a python preprocessing script, which reads the parquet, converts all the users id into a monotonic integers for the simulation to use them as index of the array directly, and writes a binary file with the layer described in @code-binary-layout
...
The Zig side reads this file into a `Topology` struct via `std.Io.takeInt(u32, .little)` calls  to build the CSR adjacency and allocate per-user data structures. In essence, Paquet files are binary files with the structure described as the first bytes of the file, in order to make all of the contents readable. In our simpler version, we just know that the format the binary is stored is exactly the one defined here.

== Traces Output

Following from the previous section, the trace output is clearly a serialization problem. Traces are stored as different structures according to the event that represent, showing the action struct as an example in @code-action-trace.
...
The simulation outputs four traces as explained in #todo[cite the design], but every trace is written twice, as a binary and as a JSONL file. This is to avoid paying the serialization task in the hot loop of the simulation. As explained in @apx-impl-trace-io, each trace has a different buffer, and to fill that buffer with a readable human format implies spending cpu cycles to serialize binary into a JSON representation of the struct. Those happen once every time the loop gets executed, which interrupts the flow of the program.

To avoid this, the serialization to a human readable format happens after the simulation has finished and as a postprocessing step in the same program. In this context a Zig native parquet SerDes would be an advantage: the only reason to make the program force this step is to avoid losing the information if the original structure changes, which in developement could happen fairly often. The main advatage of parquet to raw binary reading and writing is that parquet also stores the header, making it independent from the original structures: a simple change as swapping the order of the field `time` and `event_id` would break a hardcoded parses, as the encoding for an `f64` is vastly different for a `u64`.

The binary traces are also used by the `construct-casacdes` importing the traces file as an external dependency, avoiding the hardcoding problem, but not guaranteeing retrocompatibility.
...
To properly understand the next section, a small introduction of the concept of cache and cache locality will be introduced. If the reader does already know this, please jump to the @apx-impl-queue.
...
Despite Zig standard library having a `PriorityQueue` data structure @zig-std-priority-queue working as a heap, a new from scratch queue has been implemented for the simulation. The reasoning is that the Zig implementation defaults to a binary leaf implementation, which underutilizes cache lines on modern hardware.
...
Regarding the timeline $cal(T)_t (u)$, an implementation detail must be mentioned to understand how a refresh is performed. In design, the refresh (see @sec-design-sources, @proc-action-handle) involves updating $t$ to the current time $t_c$, but the implementation realizes this via a double-buffer per user:
- *Active*: the heap the user pops from while scrolling. It holds posts visible within the current session window.
- *Passive*: the heap where all incoming propagated posts land, regardless of whether the user is online, offline, or mid-scroll.

A refresh is therefore a swap of the passive and active heaps — the accumulated backlog becomes the new visible window, and the drained active becomes the new passive sink. This cleanly separates the posts being consumed from the posts arriving in real time, and avoids the need to filter a single heap by arrival timestamp on every pop. 
...
#todo[Carefull the distribution types here can change]
...
The pagination strategy and the library/shelves/books metaphor are introduced in @sec-design-datastructrues-post. The implementation realizes that design via the `SegmentedMultiArrayList` from the `ds` package, built on the power-of-two indexing described in @apx-impl-pow2.
...
The impression and interaction matrices $cal(E)$ and $cal(H)$ — central to the CTIC desensitization check (see @sec-design-sources-propagate) — are modelled in @sec-design-datastructrues-post as paginated bitsets. The implementation realizes this via the `PagedBitSet` from the `ds` package, applying the same power-of-two indexing from @apx-impl-pow2. The column dimension (posts) is partitioned into pages of $2^n$ columns each; the row dimension (users) is packed bitwise within each page, producing the indexing shown in @code-pagedbitset.
