...
- *Python 3.13:* main tool for data extraction, EDA, and plotting. Key packages: `polars` (columnar DataFrame engine), `numpy`, `scipy` (distribution fitting, sparse matrices, SVD), `matplotlib`, `powerlaw` (Clauset et al. MLE power-law fitting) @alstott2014powerlaw, `distfit` (parametric distribution fitting via RSS ranking) @distfit-pypi @distfit-parametric, `kneed` (elbow detection for session thresholding), `atproto` (Bluesky API client), and `python-dotenv`.
- *R 4.5.3:* per-user maximum-likelihood distribution fitting. Dependencies: `poweRlaw`, `fitdistrplus` @fitdistrplus-cran, `actuar` @actuar-cran, `evd` @evd-cran, `data.table`, `tidyverse`, `broom`, and `parallel`. #todo[Versions of `actuar`, `fitdistrplus` and `evd` not verifiable locally (no R installation with the packages); current CRAN releases as of 2026-08-11: actuar 3.3-7, fitdistrplus 1.2-6, evd 2.3-7.1]
...
The Discrete-Event Simulation engine (see @sec-design and @apx-impl) is built with Zig and a small set of custom, single-purpose libraries:
...
- *ds-bskysim:* custom Zig library providing the `PagedBitSet` and `SegmentedMultiArrayList` data structures described in @apx-impl-datastructures and @apx-impl-posts. Published under the MIT license @soler2025dsbskysim.
