...
*Zero-duration sessions*: the zero-duration sessions observed in the metrics are almost entirely singletons (40.7% of the 44.9M sessions): a singleton is a single flagged event whose span is zero by construction. Genuine multi-event sessions of zero duration are nonexistent (0 out of 44.9M sessions). The mechanism that would produce them is real ---scripted mass-follow actions that emit whole batches under one identical `createdAt`--- but the clustering collapses every such batch to a single point and flags it as a singleton: 5.6% of events share an instant with another event of the same user, of which 33% are follows.
