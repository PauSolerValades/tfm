...
fn schedulePostsUserTimelines(gpa: Allocator, rng: Random, graph: *gn.StaticNetworkGraph, simconf: SimConfig) !void {
    // START: post scheduling
    var post_count: u32 = 0;
    for(0..graph.users.len) |uid| {
        for (0..simconf.max_post_per_user) |_| {
            const follower_start = graph.users.items(.follower_start)[uid];
            const follower_count = graph.users.items(.follower_count)[uid];

            const current_pid = post_count;

            const new_post_time = simconf.diffusion_post_schedule.sample(rng);
            graph.user_seen_post.set(uid * simconf.max_post_per_user + current_pid);
            ...
            for (graph.followers[follower_start..follower_start+follower_count]) |fid| {
                ...
                const pe = entities.TimelineEvent{ 
                    .post_id = current_pid,
                    .time = new_post_time + propagation_delay,
                };
                try graph.timelines[fid].add(gpa, pe);
            }
...
pub fn diffusionSimulation(gpa: Allocator, rng: Random, simconf: SimConfig, graph: *gn.StaticNetworkGraph, trace: *Io.Writer) !SimResults {
    ...
    try schedulePostsUserTimelines(gpa, rng, graph, simconf); 
        ...
        switch (current_event.type) {
            .repost => {
                ...
                for (graph.followers[follower_start..follower_start+follower_count]) |fid| {
                    ...
                    const matrix_index = fid * simconf.max_post_per_user + p_id; 
